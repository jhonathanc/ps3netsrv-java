package com.jhonju.ps3netsrv.server.io;

import java.io.IOException;
import java.io.InputStream;
import com.jhonju.ps3netsrv.server.charset.StandardCharsets;

public final class EncryptionKeyHelper {

    private EncryptionKeyHelper() {
        // Utility class, no instantiation
    }

    public static byte[] parseKeyFromStream(InputStream inputStream) throws IOException {
        byte[] buffer = new byte[64];
        int bytesRead = inputStream.read(buffer);

        if (bytesRead < 0) {
            throw new IOException("Error: Redump key file is empty or invalid.");
        }

        if (bytesRead == 16) {
            // Raw 16-byte key
            byte[] key = new byte[16];
            System.arraycopy(buffer, 0, key, 0, 16);
            return key;
        } else if (bytesRead >= 32) {
            // Hex-encoded key
            String hexStr = new String(buffer, 0, bytesRead, StandardCharsets.US_ASCII).trim();
            if (hexStr.length() >= 32) {
                return hexStringToBytes(hexStr.substring(0, 32));
            } else {
                throw new IOException("Error: Redump key hex string is too short: " + hexStr.length());
            }
        } else {
            throw new IOException("Error: Redump key size is invalid: " + bytesRead);
        }
    }

    private static byte[] hexStringToBytes(String hex) {
        byte[] bytes = new byte[16];
        for (int i = 0; i < 16; i++) {
            int index = i * 2;
            bytes[i] = (byte) ((Character.digit(hex.charAt(index), 16) << 4)
                    + Character.digit(hex.charAt(index + 1), 16));
        }
        return bytes;
    }

    public static boolean isKeyEmpty(byte[] key) {
        for (byte b : key) {
            if (b != 0) {
                return false;
            }
        }
        return true;
    }
}
